...
from Proposed import get_yolo_results
import json
import pickle
...
#fuse = torch.jit.load("Saved models/Baseline_resnet_fullmodel_croppedEpoch5cj.pt")
fuse = torch.jit.load("Saved models/fixedBaseline_resnet_fullmodel_cropped15epochscarl_ECD.pt")
fuse.to(device)
fuse.eval()
...
def predict_image(img_path):
    ...
    with torch.no_grad():
        ...
        outputs = fuse(img_tensor_resnet)
        end_time = time.time()
...
def baseline_resnet_get_yolo_preds(image_path: str, output_img_path: str, blurredoutput_img_path: str, ):
    ...
    # Load the saved YOLO output
    with open("yolo_output.pkl", "rb") as f:
        results, yolo_elapsed_time = pickle.load(f)

    # results = output_data["results"]
    # yolo_elapsed_time = output_data["elapsed_time"]
    ...
    for i, (x, y, w, h) in enumerate(boxes):
        ...
        print(f"Person {person} classified as: {label['predicted_class']} with confidence: {label['confidence']}%")
    ...
    predictions.append({"output_image_path": bluroutput_img_path})
...
if __name__ == '__main__':
    ...
    blurredbaseline_resnet_output_filename = f"blurredBRoutput_{uuid.uuid4()}.jpg"
    ...
    baseline_resnet_get_yolo_preds("./DARKNET/3.jpg", baseline_resnet_output_path, blurredbaseline_resnet_output_path)
